"""Versioned packages and pure PalmSens workflow planning."""
...
def build_aurora_protocol(source_mode: str, source_payload: dict[str, Any] | str):
    if source_mode == "aurora_visual":
        from .visual import build_protocol_from_visual_data
...
def build_aurora_methodscript(
    ...
    return protocol.to_palmsens_methodscript(
        ...
        # PalmSens exposes each physical channel as a separate one-channel device.
        channel=0,
...
def render_aurora_step_actions(
    ...
    *,
    ...
    actions = []
    for execution_index, source_step_index, step in _expanded_protocol_steps(
        protocol, max_executed_steps
    ):
...
def _expanded_protocol_steps(
    ...
    tag_indexes = {
        step.tag: index for index, step in enumerate(method_steps) if isinstance(step, Tag)
    }
...
def save_aurora_package(path: Path | str, package: AuroraMethodPackage) -> None:
    Path(path).write_text(json.dumps(package.to_dict(), indent=2), encoding="utf-8")
